/* 
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */


document.querySelector("#cont").style.top = alto + "em";
document.querySelector("#cont").style.width = "5em";
document.querySelector("#cont").style.height = "5em";

var angolo = 0;

document.querySelector("body").onkeydown = function (e) {
    //document.querySelector("#aereo").innerHTML = e.which;
    switch (e.which) {
        case 38:
            if (e.shiftKey) {
                ridimensiona("#cont", 0, -5);
            }
            if (!e.shiftKey && !e.ctrlKey) {
                muovi("#cont", 0, -5);
            }

            break;
        case 40:
            if (e.shiftKey) {
                ridimensiona("#cont", 0, +5);
            }

            if (!e.shiftKey && !e.ctrlKey) {
                muovi("#cont", 0, +5);
            }
            break;
        case 39:
            if (e.shiftKey) {
                ridimensiona("#cont", +5, 0);
            }
            if (!e.shiftKey && !e.ctrlKey) {
                muovi("#cont", +5, 0);
            }
            if (e.ctrlKey) {
                ruota("#cont", +5);
            }
            break;
        case 37:
            if (e.shiftKey) {
                ridimensiona("#cont", -5, 0);
            }
            if (!e.shiftKey && !e.ctrlKey) {
                muovi("#cont", -5, 0);
            }
            if (e.ctrlKey) {
                ruota("#cont", -5);
            }
            break;
    }
    console.log(e);
};



function muovi(str, x, y) {

    let posX = parseFloat(document.querySelector(str).style.left);
    let posY = parseFloat(document.querySelector(str).style.top);
    posX += x;
    posY += y;
    if (posY > window.innerHeight) {
        posY = 0;
    }
    if (posY < 0) {
        posY = window.innerHeight;
    }
    if (posX > window.innerWidth) {
        posX = 0;
    }
    if (posX < 0) {
        posX = window.innerWidth;
    }
    document.querySelector(str).style.left = posX + "em";
    document.querySelector(str).style.top = posY + "em";

    console.log(str, posX, posY);
}

function ridimensiona(str, x, y) {

    let posX = parseFloat(document.querySelector(str).style.width);
    let posY = parseFloat(document.querySelector(str).style.height);
    posX += x;
    posY += y;
    document.querySelector(str).style.width = posX + "em";
    document.querySelector(str).style.height = posY + "em";

    console.log(str, posX, posY);
}

function ruota(str, a) {
    angolo += a;
//    let stringa="rotate(" + angolo + ")"
    document.querySelector(str).style.transform = "rotate(" + angolo + "deg)";
}